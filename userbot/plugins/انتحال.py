...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(event, f"**فشل الانتحال بسبب الخطأ:**\n__{e}__")
    await event.client(functions.photos.UploadProfilePhotoRequest(pfile))
    await edit_delete(event, "**⪼ تم انتحال هذا المستخدم**")
    if BOTLOG:
...
async def revert(event):
    ...
    await edit_delete(event, "**⪼ تم اعادة حسابك الى وضعه السابق**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#اعادة\n⪼ تم اعادة حسابك الى وضعه السابق",
        )
