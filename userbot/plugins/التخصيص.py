...
async def custom_jmthonuserbot(event):
    ...
    if input_str == "صورة الحماية" or input_str == "صورة الحمايه" or input_str == "صوره الحماية" or input_str == "صوره الحمايه":
        urls = extractor.find_urls(reply.text)
        if not urls:
            return await edit_delete(event, "**⪼ يجب عليك الرد على رابط تلجراف اولا**", 5)
        text = " ".join(urls)
