...
@jmthon.ar_cmd(pattern="تغيير بايو ([\s\S]*)")
async def _(event):
...
@jmthon.ar_cmd(pattern="تغيير اسم ([\s\S]*)")
async def _(event):
    ...
    try:
        ...
        if photo:
            ...
            if photo.endswith((".mp4", ".MP4")):
                ...
                if size > 2097152:
                    await jmthonevent.edit("⌔∮ الحجم يجب ان يكون اقل من 2 ميغا")
                    os.remove(photo)
                    return
                jmthonpic = None
...
@jmthon.ar_cmd(pattern="تغيير معرف ([\s\S]*)")
...
@jmthon.ar_cmd(pattern="ازاله الصورة ?([\s\S]*)")
