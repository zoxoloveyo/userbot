...
class JmthonBroadcast(BASE):
    __tablename__ = "jmthonbroadcast"
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, JmthonBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


JmthonBroadcast.__table__.create(checkfirst=True)

JMTHONBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with JMTHONBROADCAST_INSERTION_LOCK:
        broadcast_group = JmthonBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with JMTHONBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(JmthonBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with JMTHONBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(JmthonBroadcast.keywoard)
            .filter(JmthonBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(JmthonBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(JmthonBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(JmthonBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(JmthonBroadcast).all()
        for x in all_groups:
