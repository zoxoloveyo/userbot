...
def rm_from_collectionlist(keywoard, contents):
    with JMTHON_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Jmthon_GlobalCollection).get(
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Jmthon_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Jmthon_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Jmthon_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Jmthon_GlobalCollection).all()
        for x in all_groups:
