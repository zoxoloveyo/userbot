...
class JmthonGloballist(BASE):
    __tablename__ = "jmthonglobal_list"
    ...
    def __repr__(self):
        return "<Jmthon global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, JmthonGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


JmthonGloballist.__table__.create(checkfirst=True)

JMTHONGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with JMTHONGLOBALLIST_INSERTION_LOCK:
        broadcast_group = JmthonGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with JMTHONGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(JmthonGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with JMTHONGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(JmthonGloballist.keywoard)
            .filter(JmthonGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(JmthonGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(JmthonGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(JmthonGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(JmthonGloballist).all()
        for x in all_groups:
